...
const SubCategoryCard = ({ handleToggle, fetchSubCategories, isEdit, editData, loading, setLoading, categories }) => {
  const [data, setData] = useState(initialState);
  

  useEffect(() => {
    if (isEdit) {
      setData({...editData, categoryId: editData?.categoryId?._id});
    } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let res;
      if (isEdit) {
        res = await axios.post(`${API_URI}/api/subcategory/update/${editData._id}`, formData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("estate_admin_token")}`
          }
        });
      } else {
        res = await axios.post(`${API_URI}/api/subcategory/add`, formData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("estate_admin_token")}`
          }
        });
    ...
    <div className="bg-white shadow rounded-lg overflow-hidden max-w-xl mx-4 w-full absolute">
      ...
      <div className="flex flex-col gap-4 px-8 my-6 max-h-[80vh] overflow-y-auto">
        <div className="flex flex-col gap-1">
          <label className="text-sm text-gray-500">SubCategory Name*</label>
          <input
            name="name"
            value={data?.name}
